...
class Command(BaseCommand):
    ...
    def import_row(self, row, **kwargs):
        ...
        field_label = "Votre musée : {{hidden:nom}}  Y a-t-il une marche (ou plus) pour y rentrer ? (même toute petite)"
        ...
        field_label = "Combien de marches y a-t-il pour entrer dans votre musée ?"
        if field_label in row:
            if row[field_label] != "":
                try:
                    int(row[field_label])
        ...
        field_label = "Est-ce qu'il faut, pour entrer dans le musée, monter les marches ou les descendre ?"
        if field_label in row:
        ...
        field_label = "Avez-vous une rampe d'accès pour entrer dans votre musée ?"
        if field_label in row:
        ...
        field_label = "Est-ce qu’il y a des toilettes adaptées dans votre musée ?"
        if field_label in row:
        ...
        field_label = "Avez-vous un parking réservé à vos visiteurs ?"
        if field_label in row:
