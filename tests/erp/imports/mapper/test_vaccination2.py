...
def test_skip_importing_closed(mapper, neufchateau, sample_record_ok):
    ...
    _, reason = mapper(sample_closed, today=datetime(2021, 1, 2)).process()

    assert "Centre fermé le 2021-01-01" in reason
...
def test_skip_importing_reserve_pros(mapper, updates, neufchateau, sample_record_ok):
    ...
    _, reason = mapper(sample_reserve_pros).process()

    assert RAISON_RESERVE_PS in reason
...
def test_skip_importing_reserve_public_restreint(
    ...
    _, reason = mapper(sample_public_restreint).process()

    assert RAISON_PUBLIC_RESTREINT in reason
...
def test_save_non_existing_erp(mapper, neufchateau, sample_record_ok):
    ...
    assert erp.source_id == "219"
    assert erp.activite == VaccinationMapper.activite  # todo
...
def test_unpublish_closed_erp(mapper, neufchateau, sample_record_ok):
